...
function [ref_time, V, T] = remapping_time(folder,h,signal_processing_parameters,EC_reconstruction_parameters)
...
for j = 1:length(name)
    ...
    min_time = min(min_time,time_pad(1));
    max_time = max(max_time,time_pad(end));    
...
%             V{end}( jj, ismember(ind_vec(:,jj),ind)) = 1/2;
        end
